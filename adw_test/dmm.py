...
class EarlyStopping:
    ...
    def print_info(self, info):
        ...
        if save_path is None:
            ...
            print(
                "epoch %d, training cost %g (error=%g), validation cost %g (error=%g) (count=%d) "
                % format_tuple
            )
            print(
                "[LOG] %d, %g,%g,%g,%g, %g, %g,%g,%g, %g,%g,%g"
                % (
                    epoch,
                    training_cost,
                    validation_cost,
                    ...
                    validation_error,
                    alpha,
                    training_all_costs[0],
                    training_all_costs[1],
                    training_all_costs[2],
                    validation_all_costs[0],
                    validation_all_costs[1],
                    validation_all_costs[2],
                )
            )
        else:
            format_tuple = (
            ...
            print(
                "epoch %d, training cost %g (error=%g), validation cost %g (error=%g) (count=%d) ([SAVE] %s) "
                % format_tuple
            )
            print(
                "[LOG] %d, %g,%g,%g,%g, %g, %g,%g,%g, %g,%g,%g"
...
def filter_discrete_forward(sess, config):
    ...
    params = computeEmission(
        z_holder, n_steps, init_params_flag=True, control_params=control_params
    )
