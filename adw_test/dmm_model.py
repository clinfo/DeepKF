...
def p_filter(
    x, z, epsilon, sample_size, proposal_sample_size, batch_size, control_params
):
    hy_param = hy.get_hyperparameter()
    dim = hy_param["dim"]
    dim_emit = hy_param["dim_emit"]
    ...
    if ptype == "trained_dynamics":
        if dytype == "function":
            ...
            out_params = computeTransitionDistWithNN(
                z, 1, init_params_flag=True, control_params=control_params
            )
            if sttype == "discrete":
...
def loss(outputs, alpha=1, control_params=None):
    ...
    mean_cost = tf.reduce_mean(
        negCLL + alpha * temporalKL + alpha * 1.0 * cost_pot, name="train_cost"
    )
