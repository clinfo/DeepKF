...
def plot_filter(filter_jbl,filter_image):
    ...
    for i in range(100):
        ...
        x_image = ax2.plot(x,asymmetric_double_well_energy(x),marker="o",color="blue",linestyle='None')
        #vline = ax3.axvline(x=i,ymin=-1.0,ymax=5.0)
        #x_time = ax3.plot(i,x,marker="o",color="blue",linestyle="None")
        itr = [ax1.text(2.0,5.0,"step : "+str(i))]
        artists.append([z_image]+x_image+itr)
        #artists.append([z_image]+x_image+[vline]+itr)
