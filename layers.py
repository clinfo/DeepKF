...
def embedding_layer(name,input_data,dim_dict,dim_features,init_params_flag=True,params=None):
...
def gcn_layer(name,features,adjs, dim_in,dim_out,node_num,adj_channel_num,batch_size, init_params_flag=True,params=None):
	if not init_params_flag:
		tf.get_variable_scope().reuse_variables()
	wd_b=None
	wd_w=0.1
...
def graph_fc_layer(name,data,dim_in,dim_out,node_num,wd_w,wd_b,activate,with_bn=False,init_params_flag=True,params=None):
	if with_bn:
		layer=tf.reshape(data,(-1,dim_in))
		layer=fc_layer(name,layer,dim_in, dim_out,wd_w,wd_b,activate=None,with_bn=False,init_params_flag=init_params_flag)
		layer=tf.reshape(layer,(-1,node_num,dim_out))
		layer=graph_batch_normalization(name+"/bn",layer,dim_out,node_num,init_params_flag)
		if activate is None:
			return layer
		else:
		...
		layer=tf.reshape(data,(-1,dim_in))
		out=fc_layer(name,layer,dim_in, dim_out,wd_w,wd_b,activate,with_bn=False,init_params_flag=init_params_flag)
