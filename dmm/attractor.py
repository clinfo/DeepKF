...
def construct_default_placeholder(config):
    ...
    #
    placeholders = {"dropout_rate": dropout_rate, "is_train": is_train}
    return placeholders
...
def compute_discrete_transition_mat(sess, config):
    ...
    if config["dynamics_type"] == "distribution":
        params = computeTransitionDistWithNN(
            z_holder, n_steps, control_params=control_params
        )
        z_m = params[0]
...
def field_continuous(sess, config):
    ...
    if config.get("field_grid_dim") is None:
        config["field_grid_dim"] = dim
    if config.get("field_grid_range") is None:
...
def potential(sess, config):
    ...
    # inference
    # z_m,_,_=computeTransition(z_holder,n_steps,dim,mean_prior0=None,cov_prior0=None,params=None,without_cov=True)
    z_m, _ = computePotential(
        z_holder, n_steps, dim, params=None, control_params=control_params
    )
...
def infer(sess, config):
    ...
    placeholders["z"] = z_holder
    # inference
    z_m, _ = computeTransitionFunc(
        z_holder, n_steps, dim, params=None, control_params=control_params
    )
    z_m = tf.reshape(z_m, [-1, dim])
    obs_params, params_e = computeEmission(
        z_holder, n_steps, dim, dim_emit, params=None, control_params=control_params
    )
    obs_params2, params_e = computeEmission(
        z_m, n_steps, dim, dim_emit, params=params_e, control_params=control_params
    )
