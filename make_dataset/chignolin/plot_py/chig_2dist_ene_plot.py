...
for name in list:
    ...
    atom_pairs=np.array([[46, 119], [44, 105]])
...
#
    ene=name+'/protein_gpu/equil_n1/energy.xvg'
    x, y = get_xy(ene)
    ene_arr=np.array(y[::50])

    plt.figure()
#    plt.title('Dihedral Map: Alanine dipeptide')
    plt.scatter(dist[:, 0], dist[:, 1], s=10, alpha=0.7, c=ene_arr)
    cbar = plt.colorbar()
    ...
    plt.ylim(0, 2)
    plt.savefig(name+"_2dist_ene_plot.png")
