...
def prinz_gradient(x):
    return 4 * (
        8 * x ** 7
        - 128.0 * np.exp(-80 * x ** 2) * x
        - 32.0 * np.exp(-80 * (x - 0.5) ** 2) * (x - 0.5)
        - 40 * np.exp(-40.0 * (x + 0.5) ** 2) * (x + 0.5)
    )
...
class FoldingModel(BrownianDynamics):
    ...
    def __init__(self, dt, kT, mass=1.0, damping=1.0, rcut=3.0):
        super(FoldingModel, self).__init__(2, dt, kT, mass, damping)
        self.rcut = rcut
...
def get_asymmetric_double_well_data(
    nstep, x0=0.0, nskip=1, dt=0.01, kT=10.0, mass=1.0, damping=1.0
):
...
def get_prinz_pot(nstep, x0=0.0, nskip=1, dt=0.01, kT=10.0, mass=1.0, damping=1.0):
...
def build_generator_on_source(data_source, batch_size, lag, output_size):
    ...
    data_iterator = data_source.iterator(
        chunk=batch_size, lag=lag, return_trajindex=False
    )
